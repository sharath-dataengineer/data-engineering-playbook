...
**What this is.** Project Tungsten is the execution-engine rewrite that moved Spark to off-heap binary rows (`UnsafeRow`) and whole-stage code generation. This chapter explains the binary format, whole-stage codegen, and how to keep the engine engaged so your CPU-bound stages stay fast.
...
- Explain how `UnsafeRow` and whole-stage codegen turn a Volcano iterator chain (a query model where each operator calls the next one row-by-row) into one JIT-friendly loop (a tight loop the Java compiler can heavily optimize), and read the `*(n)` prefix in `explain()`.
...
- Tungsten is the execution-engine rewrite (Spark 1.4–2.0) that moved Spark from a JVM-object (Java Virtual Machine object — the standard way Java programs represent data in memory), iterator-per-row model to **off-heap binary rows** (`UnsafeRow`) and **whole-stage code generation** (WSCG). The premise: once shuffle and I/O were optimized, the bottleneck became the CPU — branch misprediction, virtual dispatch, and GC (garbage collection — the JVM process that reclaims unused memory), not the disk.
- `UnsafeRow` stores a row as a flat byte buffer (null-bitset + fixed-length region + variable-length region) so Spark can compare, hash, and copy rows without ever deserializing into JVM objects. This is what makes sort and hash-aggregate operate at near-`memcpy` (raw memory copy) speeds.
...
- The `TungstenAggregate` (`HashAggregateExec`) uses an off-heap `BytesToBytesMap` (a hash map stored in raw memory outside the JVM heap) and falls back to a sort-based aggregate under memory pressure — the fallback is a real, observable cliff in production.
- Codegen is not free: methods over **8KB bytecode** stop being JIT-compiled by HotSpot (the JVM's just-in-time compiler, which converts bytecode to fast native machine code), the **64KB method limit** forces a fallback to the interpreted code path, and very wide schemas (hundreds of columns) routinely defeat it. Knowing *when codegen silently turns off* is the senior skill.
...
2. **Cache:** lay out data so the hot path is sequential and cache-line friendly (data fits neatly into the CPU's fast cache memory), and use `sun.misc.Unsafe` for direct memory access.
3. **CPU:** generate code at runtime so the JIT sees a flat loop it can inline and vectorize (process multiple values at once using CPU-level parallelism), instead of a megamorphic chain (a call site that could point to many different implementations, which the CPU cannot predict) of `Iterator.next()` calls.

The payoff on that aggregation is often **2–10×** on CPU-bound stages and a collapse of GC time to single digits. If you've ever wondered why a Spark 2.x job crushed the equivalent Spark 1.3 job on identical hardware, this is most of the answer. It pairs directly with Catalyst — Catalyst decides *what* the plan is; Tungsten decides *how fast the bytes move through it*.
...
An `UnsafeRow` is a contiguous byte region (a single, uninterrupted block of memory) with three parts:
...
- The **fixed-length region** stores every field in exactly 8 bytes. Primitives (`long`, `double`, `int` widened to 8) sit here directly. For variable-length types (`String`, `binary`, `array`, `map`, `struct`), this slot holds an **offset+length pointer** (high 32 bits = offset into the variable region, low 32 bits = length of the value) into the variable region.
...
- **Compare** two rows for sort by comparing prefix bytes (radix/prefix sort on the 8-byte sort-key prefix — important for performance under skew).
- **Hash** a row by hashing the byte range directly (Murmur3, a fast hash algorithm, over the buffer).
- **Copy/spill** a row with a single `Platform.copyMemory` — no field-by-field serialization.

The practical consequence: sort, hash-aggregate, and shuffle all operate on opaque byte ranges. There is no per-field boxing (wrapping a primitive like `long` into a Java object like `Long`) on the hot path.

### Whole-stage code generation (WSCG)

The classic "Volcano" model executes a query as a tree of iterators, each calling `next()` on its child. For a `Filter → Project → Aggregate` chain over a billion rows, that's billions of polymorphic virtual calls (calls where the JVM has to look up which method to actually run at each step) — the CPU can't predict the branch targets, the JIT can't inline across the iterator boundary, and you pay a function-call overhead per row per operator.
...
- **The 64KB JVM method-size limit.** Generated methods cannot exceed 64KB of bytecode (the intermediate format the JVM runs before the JIT compiles it to native code). Wide queries — hundreds of projected columns, deeply nested `CASE WHEN`, large `IN` lists — blow past this. Spark catches the `CompileException` / `JaninoRuntimeException` and falls back to the interpreted code path for that whole stage. You'll see a warning like `Whole-stage codegen disabled for this plan ... grows beyond 64 KB` in the executor logs, then your "fast" stage runs 3–5× slower with no other signal.
...
- **The 8KB JIT threshold.** HotSpot's `-XX:-DontCompileHugeMethods` default refuses to JIT methods over 8000 bytes of bytecode. A generated method can be valid (under 64KB) but too big to JIT — so it runs interpreted, which can be *slower than no codegen at all*. This is the subtle one: codegen "worked" but the JIT punted (skipped compilation of that method entirely).
...
People conflate "Tungsten uses off-heap" with "Spark runs off-heap by default." It does not. Off-heap memory means memory allocated outside the JVM heap, so it is never touched by garbage collection. By default Tungsten's binary buffers live in **on-heap** memory managed by the unified `MemoryManager`. True off-heap (`sun.misc.Unsafe.allocateMemory`) only kicks in when you set:
...
- `collect_list` / `collect_set` / `percentile` are **typed imperative aggregates** (aggregates implemented in full JVM objects rather than the binary map) that don't use the binary hash map at all — they route to `ObjectHashAggregateExec` and are far heavier. Watch for these in plans.

### 4. Codegen interacts with the row format — and UDFs break the chain

A Scala/Python UDF (user-defined function — custom code you write and register with Spark) is a black box to the code generator. When a stage hits a UDF, Spark must **deserialize the `UnsafeRow` into JVM objects** (convert the raw binary row back into regular Java/Python objects), call your function, and re-serialize. That serialize/deserialize boundary defeats the entire point of Tungsten for that operator. A PySpark UDF is worse: it ships rows out to a Python worker process (until Arrow-based pandas UDFs, which at least batch and zero-copy via Arrow — meaning data is shared between JVM and Python without copying it). Rule of thumb: **every plain UDF is a Tungsten off-ramp.** Prefer built-in expressions (which are codegen-aware via `Expression.doGenCode`) or `pandas_udf` with Arrow.

### 5. Vectorized Parquet reader is part of the same story

The `VectorizedParquetRecordReader` (`spark.sql.parquet.enableVectorizedReader`, default `true`) reads Parquet into `ColumnarBatch` / `ColumnVector` (off-heap column buffers — memory that stores an entire column at once rather than row-by-row) and feeds them into codegen in batches of `spark.sql.parquet.columnarReaderBatchSize` (default 4096) rows. This is columnar-in, row-codegen-out, and it's why scan stages are so fast. Reading complex nested types or certain decimal layouts can disable the vectorized path and fall back to row-by-row — another silent slowdown to watch for.
...
- **Project narrow, early.** Push column pruning (selecting only the columns you need) ahead of joins/aggregates so wide-schema fallbacks (`maxFields`, 64KB) never trigger. Selecting 12 columns out of 300 before a `groupBy` is the difference between codegen on and off.
...
- This complements Catalyst (logical/physical planning), AQE (Adaptive Query Execution — runtime re-planning), and broadcast-join (which join strategy feeds the codegen stages).

## Further reading

- Sibling chapters: Catalyst optimizer · Adaptive Query Execution · Broadcast joins · Partitioning · Skew handling
