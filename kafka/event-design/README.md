...
**What this is.** The shape of an event — its schema, key, partitioning, retention, and envelope — is a long-lived API contract. Think of it like a public API: once other teams start depending on it, changing it becomes expensive. This chapter covers how to design events that survive years of producers and consumers evolving independently without coordinated deploys.
...
- Choose deliberately between fact, delta, and state events and understand the replay, idempotency (the ability to safely apply the same operation more than once), and compaction properties of each.
- Pick a partition key that gives you the ordering you need while managing hot-spot and parallelism trade-offs, and configure `FULL_TRANSITIVE` schema evolution (a strict compatibility mode explained below).
- Design a stable event envelope for routing, dedup (deduplication — filtering out duplicate messages), and lineage, and run dual-write migrations for semantically breaking changes.
...
- An event is a **contract**, not a message. Treat the schema, the key, the partitioning, the compaction semantics (how old data gets cleaned up), and the topic naming as one indivisible API surface that you version deliberately.
...
- The **partition key is your ordering guarantee and your hot-spot risk**. Kafka only orders messages within a single partition. Pick the key that matches the aggregate (the entity or object) you need ordered (usually the entity ID), and accept that you've now coupled throughput to key cardinality (the number of distinct key values).
- **Schema evolution is backward/forward compatibility math**, not guesswork. Adding an optional field with a default is safe; removing a required field, changing a type, or renaming is not. The Schema Registry (a centralized service that stores and validates schemas) enforces this — configure `FULL_TRANSITIVE` and mean it.
...
- Design for **replay from offset 0** (reprocessing all messages from the very beginning of the topic) as a first-class operation. If reprocessing the topic from the beginning produces a different result than the live stream did, your events aren't really events — they're RPC calls in a trench coat.
...
Then a producer team "optimized" by switching to **delta** events (`amount_delta` instead of `amount`) to save bandwidth, behind a feature flag, without a version bump. This change was backward-compatible at the Avro (a binary serialization format) level — they added a nullable field and removed nothing. The Schema Registry passed it. Production broke in three different ways within an hour:
...
The schema was compatible. The **semantics** (the meaning of the data) were not. Compatibility checks validate that bytes deserialize (can be read without crashing); they do not validate that meaning is preserved. That gap — between wire compatibility and semantic compatibility — is where most streaming incidents live, and it's why event design is a principal-level concern, not a serialization detail.
...
**Partition assignment.** The producer computes `partition = murmur2(key) % num_partitions` (the default `DefaultPartitioner`). This is a hash function that consistently maps the same key to the same partition number. Same key → same partition → total order for that key. Null key → round-robin (sticky in modern clients), so **null-keyed topics have no per-entity ordering at all**. This is the single most load-bearing fact in event design: your ordering guarantee is exactly "per key, within a partition, never across."

**Compaction vs. retention.** There are two models for how Kafka cleans up old data, and the event type dictates which one to use:
...
Compaction is a key-by-key "keep the last write" garbage collection. A `null` value for a key is a **tombstone** (a special marker that signals deletion) — it deletes the key after `delete.retention.ms`. This means state topics and tombstones are a coupled design: if your event type is "state," deletes must be a value of `null`, not a `deleted=true` field, or compaction will never reclaim the key.

**Compatibility math.** For a registry compatibility mode, the question is whether reader schema R can read data written with writer schema W. Here is what each mode means in plain terms:
...
- **FULL**: both directions work. Safe changes: add/remove only fields *with defaults*.
- **`_TRANSITIVE`**: the above rules are checked against **all** prior versions, not just the immediately previous one. Without transitive, you can chain three "compatible" steps into a net-incompatible jump.
...
This is the most important choice in event design, because it affects every downstream consumer.
...
The trap: **deltas look efficient and behave like landmines.** A redelivery (which Kafka allows under at-least-once delivery) double-applies the change. A consumer that skips one offset is permanently desynchronized with no signal. Use deltas only when you have an independent reconciliation mechanism (e.g., periodic full snapshots into the same topic, or a separate state topic to true up against). For most domains, **emit facts, and let consumers fold facts into their own state** — that pushes the materialization (the step of building a view from raw events) where it belongs, into the consumer that knows what state it needs.

### The envelope

Routing, dedup, and lineage should never require parsing domain fields. Standardize an envelope (a wrapper that carries metadata about the event, separate from the business payload) and keep it stable across all topics:
...
`id` powers consumer-side dedup (store seen IDs in a TTL'd RocksDB/state store, where TTL means the IDs expire after a set time). `causationid`/`traceparent` give you event lineage across services — when the fraud team asks "what triggered this?", you can walk the chain. `dataschema` lets a consumer pin the exact contract it parses without trusting the topic name. Crucially, **put `type` and `id` in the Kafka record headers too**, so routers and dedup filters never deserialize the payload at all.

### Schema design choices that bite later

- **Avro over JSON for high-volume topics.** Avro is a binary format that stores schemas separately and enforces compatibility through the registry. JSON Schema is fine for low-volume control planes where human-readability wins. Protobuf (another binary format from Google) is the right call when you also have gRPC services sharing the contract.
...
- **Logical types and time.** Always store time as `timestamp-millis` (UTC epoch — milliseconds since January 1, 1970), never a formatted string. Capture both **event time** (when it happened in the domain) and **ingestion time** (when produced to Kafka) — windowed consumers and Flink/Spark watermarking (the mechanism stream processors use to track event-time progress) need event time, and you cannot reconstruct it later.
...
- Key by the **aggregate root you need ordered** (the top-level entity that owns related data). For payments that's usually `account_id`; for IoT it's `device_id`. If you key by something high-cardinality and uniform (`event_id`), you get perfect spread but **zero ordering** — fine for facts, fatal for anything order-dependent.
- **Hot keys throttle a whole partition.** One `account_id` doing 50% of traffic pins one partition's consumer at 100% while 23 others idle. Detect with per-partition lag skew (the difference in how far behind each partition's consumer is). Mitigations: composite key (`account_id + bucket`) if you can relax to per-account *eventual* ordering, or pull the hot entity onto its own topic.
- **Partition count is sticky.** You can increase partitions, but doing so **changes `key % N` and breaks all existing per-key ordering** from that moment — historical messages for a key may now live on a different partition than new ones. Over-provision partitions at creation (a 2-4x headroom on expected consumer parallelism) rather than reshard live. This interacts directly with consumer scaling — see the consumer-groups chapter.
...
- **Carry trace context in the envelope** so a single user action's event chain is walkable across services — this is your incident-debugging lifeline and feeds the observability chapter.
- **Pair every topic with a DLQ contract.** Poison messages (unparseable or failed-invariant messages) go to a dead-letter topic (DLQ — a separate topic that catches problematic messages for later inspection) with the original headers plus a failure reason — see the dlq chapter.
...
| No `event_id` for dedup | Double-processing on every consumer rebalance/redelivery | Add a stable ULID (a time-sortable unique ID format); dedup in a TTL'd state store |
...
Adjacent guarantees live in sibling chapters: ordering/replay interacts with the offsets chapter, delivery semantics with the exactly-once chapter.
...
- consumer-groups chapter — how partition count and keys cap consumer parallelism.
- offsets chapter — replay, reset-to-earliest, and what "replay from 0" actually requires.
- exactly-once chapter — idempotent producers, transactions, and why consumer dedup still matters.
- dlq chapter — the failure contract that pairs with every event contract.
- data-modeling chapter — how events land as facts/dimensions in the lakehouse.
