...
- An offset is a per-partition monotonic position, not a global clock. `offset=4815` in partition 0 has nothing to do with `offset=4815` in partition 1. Ordering guarantees stop at the partition boundary.
...
- `enable.auto.commit=true` commits on a timer (`auto.commit.interval.ms`, default 5000ms) in `poll()`, decoupled from whether your work succeeded. It is at-most-once or at-least-once depending on crash timing — never exactly-once. Most "we lost data on deploy" incidents trace back to it.
...
- `__consumer_offsets` is just a compacted Kafka topic (50 partitions by default). Offset commits are produces; offset fetches are reads of the last value per key. The group coordinator owns the partition for your `group.id`.
- For real exactly-once you don't trust offsets alone — you either store the offset **transactionally with your output** (offset = function of state), or you make writes idempotent on a business key. See [exactly-once](../exactly-once/README.md).
...
Each partition is an append-only log. The offset is the array index of a record in that log. The broker tracks several watermarks per partition that you must be able to read off a lag dashboard:
...
| **High watermark (HW)** | Highest offset replicated to all ISR; consumers can only read up to HW |
...
- `latest` (default) — start at HW. **Silently skips everything currently in the topic.** A new consumer group on a busy topic processes zero history and you wonder why your backfill is empty.
- `earliest` — start at log start offset. Reprocesses everything retained. Correct for backfills, dangerous if your processing isn't idempotent.
- `none` — throw `NoOffsetForPartitionException`. Forces you to decide explicitly. The right choice for anything where a wrong default is a correctness incident.

The retention trap: `offsets.retention.minutes` (default 7 days in modern brokers, was 1 day pre-2.0) controls how long a committed offset survives **after the group goes empty**. A consumer group that's down for a long weekend can have its offsets expire, then on restart it falls back to `auto.offset.reset` and either skips everything (`latest`) or reprocesses everything (`earliest`). I have seen a low-volume nightly job lose its place over a holiday because `offsets.retention.minutes` was shorter than the gap between runs.
...
This is **at-least-once**: if you crash after `process()` but before the commit lands, you reprocess. That's the correct default for most pipelines — pair it with idempotent writes.

### `max.poll.interval.ms` and the silent rebalance that eats your commit

`max.poll.interval.ms` (default 300000ms = 5 min) is the maximum time between `poll()` calls before the coordinator considers the consumer dead and rebalances its partitions away. If your `process()` of a 500-record batch with a slow API takes longer than 5 minutes, the broker revokes your partitions mid-batch. Your subsequent `commitSync` then throws `CommitFailedException` because you no longer own those partitions — and another consumer has already started reprocessing from the old committed offset. Symptom: duplicate processing plus a storm of `CommitFailedException` in logs during high-latency periods. Fix: lower `max.poll.records`, raise `max.poll.interval.ms`, or move slow work off the poll thread.

### Why offsets alone can't give exactly-once

The committed offset and your external write are two separate systems. There is no two-phase commit between Kafka and Postgres for free. So:
...
Two offset-specific things make this correct: `failOnDataLoss=true` turns a silently-skipped range (offsets expired) into a loud failure instead of data loss, and `txnVersion`/`txnAppId` make Delta dedupe replayed microbatches so that Spark's at-least-once Kafka read becomes effectively-once at the sink. `startingOffsets` only applies on the very first run — once the checkpoint exists, the checkpoint's offsets always win, which is why deleting a checkpoint to "reprocess from latest" is one of the most destructive operations in streaming.
...
- **For exactly-once, derive the offset from the sink.** Either Kafka transactions for read-process-write, or persist offsets in the sink's transaction. Don't bolt external 2PC onto `commitSync`.
...
- "Exactly-once across Kafka and a foreign store isn't achievable by tuning `commitSync` — there's no free 2PC. I either use Kafka transactions for the read-process-write loop, or I make the offset a function of sink state so recovery is deterministic." (Defends the [exactly-once](../exactly-once/README.md) design.)
...
- On rebalances: "Long `process()` times blow past `max.poll.interval.ms`, the coordinator revokes partitions, the next commit fails, and another consumer reprocesses. I bound `max.poll.records` and keep slow I/O off the poll thread." (See [consumer-groups](../consumer-groups/README.md).)
...
- [exactly-once](../exactly-once/README.md) — turning at-least-once offsets into end-to-end EOS with transactions and idempotent sinks.
- [consumer-groups](../consumer-groups/README.md) — rebalance protocol, partition assignment, and how revocation interacts with commits.
- [dlq](../dlq/README.md) — what to do with the poison record so you can commit past it instead of stalling the partition.
- [event-design](../event-design/README.md) — keying and partitioning choices that determine what "ordering" your offsets actually buy you.
- Kafka docs — [Consumer offset management & `__consumer_offsets`](https://kafka.apache.org/documentation/#design_consumerposition); KIP-98 (transactions / exactly-once) and KIP-211 (offset retention semantics) for the protocol-level details.
