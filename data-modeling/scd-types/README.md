...
- Type 2 is the workhorse for analytics that must reproduce historical state. Its correctness lives entirely in three plumbing columns most people get wrong: `effective_from`, `effective_to`, and `is_current`, plus a deterministic surrogate key.
- On a lakehouse (Iceberg/Delta/Hudi) the merge mechanics dominate cost and correctness. A naive `MERGE` that rewrites the whole dimension every run is the difference between a 4-minute and a 90-minute job at 200M rows.
- Late-arriving and out-of-order source changes break Type 2 in subtle ways: overlapping validity windows, lost history, and "current" flags that point at two rows. You need a deterministic ordering key, not `processing_time`.
- Hash-diff on a stable column set is how you decide *whether a change happened at all*. Hashing the wrong columns causes either missed history (under-hashing) or a new version every load (over-hashing on a volatile `updated_at`).
...
Picture a SaaS billing model. A customer signs up on `plan = 'Pro'` in a US region, upgrades to `Enterprise` six months later, and the sales territory gets re-drawn in Q3 moving them from `West` to `Mountain`. Finance runs a quarterly revenue-by-territory report. The order facts are immutable — an invoice for $1,200 was issued on a specific date. But which territory does that $1,200 belong to?
...
That `BETWEEN`-style join against validity windows is the entire payoff of Type 2. Everything else in this chapter is making that join correct, fast, and reproducible. This pairs directly with [star-schema](../star-schema/README.md): facts carry surrogate keys (`customer_sk`) that resolve to the *version-correct* dimension row at fact-load time, so the join above is usually pre-baked into the fact.

## How it works

The SCD "types" are a catalogue of strategies for one attribute when its source value changes. They are independent and composable.
...
Type 2 is the one with real mechanics. Each natural-key entity becomes a *chain* of rows, each row valid for a contiguous time interval:
...
Three invariants make the chain valid, and the entire job of an SCD2 pipeline is to preserve them:
...
That `row_hash` is the load-bearing optimization. Comparing a single 64-bit/256-bit hash beats column-by-column `IS DISTINCT FROM` chains on wide dimensions, and it's what makes the merge predicate cheap.
...
Use `[effective_from, effective_to)` — closed on the left, open on the right. The previous row's `effective_to` equals the next row's `effective_from`. The alternative — closed-closed with `effective_to` set to "the day before the next row starts" — forces you to subtract a day (or a microsecond) and immediately breaks at fine time grains, around DST, and at midnight boundaries. With half-open, the fact join is unambiguous:
...
No row can match two versions, and no event falls between versions. Set the open end to a real sentinel like `TIMESTAMP '9999-12-31 00:00:00'`, **not** `NULL`. `NULL` poisons the `<` comparison (predicate returns unknown, row drops out of the join) and you lose all current-version facts silently. This single bug — `effective_to IS NULL` for current rows joined with `<` — is the most common "my latest data disappeared from the report" incident.

### The ordering key is not processing time

Type 2 must apply changes in *business-event order*, not the order your pipeline happened to read them. If two changes for the same customer land in the same micro-batch — or worse, arrive out of order because of a Kafka rebalance or a backfill — ordering by `current_timestamp()` or ingestion time produces overlapping windows or lost intermediate versions.

You need a monotonic, source-supplied ordering key: a CDC log sequence number (LSN/SCN), a Debezium `source.lsn`/`ts_ms`, or a database `updated_at` you *trust*. When two versions share the same business timestamp, break ties deterministically (e.g., by LSN) so re-runs are idempotent. See [event-driven-systems](../../distributed-systems/event-driven-systems/README.md) for why out-of-order delivery is the default assumption, not the exception, and [offsets](../../kafka/offsets/README.md) for how CDC offsets give you that monotonic key.
...
- **Over-hashing:** including a volatile column like `last_login_at` or `updated_at` in the hash. Every load produces a "change," your dimension explodes with versions that carry no analytical meaning, and storage/query cost balloons. I've seen a 5M-row customer dimension grow to 400M rows in a month from a single `last_seen` column in the hash.
- **Under-hashing:** forgetting a tracked column. The change is silently dropped; history is wrong and there's no error to catch it. Reconciliation against source is the only way you'll find it — see [reconciliation](../../data-quality/reconciliation/README.md).
...
Do not use `monotonically_increasing_id()` in Spark for surrogate keys — it's not stable across re-runs and encodes partition IDs. Use one of:
...
Why two statements instead of one `MERGE`? A single `MERGE` cannot both `UPDATE` the old row and `INSERT` a new one for the same matched key in one pass (each target row matches at most one action). The close-then-insert split is the standard, correct decomposition. Run them in one transaction where the engine supports it; on Iceberg, wrap with a snapshot and validate invariants before commit.

A post-merge assertion I gate every run on — it catches overlap/duplicate-current bugs before they reach a report:
...
- **Pre-resolve surrogate keys at fact-load time.** Don't make BI tools do the validity-window join. When loading `fct_order`, look up the version-correct `customer_sk` for the order's event time and store it on the fact. The fact-to-dim join then becomes a plain equi-join on `customer_sk` — fast, and immune to later dimension edits. This is the link back to [star-schema](../star-schema/README.md).
- **Compaction matters more than the merge.** SCD2 is an insert-heavy, small-file-generating workload. On Iceberg, schedule `rewrite_data_files` and expire snapshots; otherwise the validity-window join degrades as the dimension fragments into thousands of small files. See [iceberg](../../lakehouse/iceberg/README.md).
...
- **Effective-dating from CDC, not snapshots, when you can.** Snapshots lose intra-day changes and can't tell a real change from a re-stated value. A Debezium CDC stream gives you `effective_from` straight from the transaction log and the ordering key for free.
...
- You have an append-only event log and can reconstruct any historical state by replaying events → you may not need a versioned dimension; derive state at query time. Compare with [data-vault](../data-vault/README.md), whose satellites are effectively SCD2 by construction with stricter load auditability.
...
- [star-schema](../star-schema/README.md) — how surrogate keys connect facts to version-correct dimensions.
- [data-vault](../data-vault/README.md) — satellites as formalized, hashdiff-driven Type 2.
- [snowflake-schema](../snowflake-schema/README.md) — normalization tradeoffs that interact with dimension versioning.
- [iceberg](../../lakehouse/iceberg/README.md) — merge mechanics, compaction, and snapshot expiry for SCD workloads.
- [reconciliation](../../data-quality/reconciliation/README.md) — catching under-hashing and dropped history against source.
- [event-driven-systems](../../distributed-systems/event-driven-systems/README.md) and [offsets](../../kafka/offsets/README.md) — out-of-order delivery and the monotonic ordering key.
- Kimball & Ross, *The Data Warehouse Toolkit* (3rd ed.), Ch. 5 — the canonical SCD type taxonomy.
- Apache Iceberg docs — [`MERGE INTO`](https://iceberg.apache.org/docs/latest/spark-writes/#merge-into) and `rewrite_data_files` maintenance procedures.
