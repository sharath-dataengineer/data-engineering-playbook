...
- Snowflaking = splitting a dimension into normalized child tables on functional dependencies (e.g. `product → category → department`). You trade storage and update simplicity for join depth at read time.
- On a columnar MPP engine (Snowflake, BigQuery, Redshift) or a lakehouse (Spark + Iceberg), the storage savings from snowflaking a dimension are usually **negligible** — dictionary encoding already collapses repeated strings. The real reasons to snowflake are *governance, update isolation, and conformance*, not bytes.
...
- Snowflaking interacts badly with [SCD Type 2](../scd-types/README.md) history: if both parent and child dimensions are versioned, you get a combinatorial explosion of effective-dated joins. Decide *which level owns history* before you normalize.
...
Concrete scenario. You own a `fact_sales` table at ~8B rows for a retail analytics platform. The product dimension is `dim_product` at ~2.4M SKUs. Merchandising restructures the category hierarchy roughly twice a quarter — categories get renamed, merged, re-parented. Marketing owns the brand attributes; merchandising owns the category tree; the SKU master comes from the PIM system.
...
Now a category rename is a single-row update in `dim_category`. SCD2 history lives at the category grain, not exploded across 2.4M SKUs. Ownership maps cleanly to tables. The cost: queries that group by category now traverse `fact → dim_product → dim_category`, two joins where flat needed zero.
...
Snowflaking is normalization applied selectively to dimensions. You take a dimension that contains transitive functional dependencies and split it on those dependencies until each table is in (at least) third normal form for its grain.

A dependency like `sku → category_id → category_name → department_id → department_name` means `department_name` does not depend on the SKU directly — it depends on it *through* category and department. In a flat star dimension you accept that redundancy (every SKU row repeats its department name). Snowflaking removes it by giving each level its own table keyed by its own surrogate.
...
If you snowflake `dim_product → dim_category` and both are SCD Type 2, you have to decide which table owns version history for a category attribute. Putting effective dates on *both* tables creates a temporal join nightmare: to reconstruct "what was the category name for this SKU on the sale date" you need:
...
Two range joins. Range joins do not broadcast cleanly and they blow up the optimizer's cardinality estimates. The fix: **history lives at exactly one grain per attribute.** Either the fact carries a point-in-time `category_key` snapshot resolved at load time (preferred — see [SCD Types](../scd-types/README.md) for the "fact-resolves-the-version" pattern), or you snapshot the whole product hierarchy into a flat SCD2 `dim_product` and don't snowflake the versioned parts at all. Snowflaking and Type-2 history are in tension; resolve it explicitly.

### 2. The BI-tool join-path trap

When you snowflake, you give BI semantic layers (Looker, Power BI, Tableau, dbt's `metricflow`) more than one way to traverse from fact to a leaf attribute. If `dim_product` joins to both `dim_category` and `dim_brand`, and `dim_category` joins to `dim_department`, a naive query generator can produce a join that fans out through the wrong path, or — worse — produce a **fan trap**: joining one-to-many at two levels and double-counting the additive measure. Symptom: `SUM(net_amount)` is suddenly inflated by a clean integer multiple and nobody knows why. The flat star is much harder to misuse this way because there's exactly one path. If you snowflake, you must pin join paths in the semantic layer explicitly (Looker `explore`/`join` graphs, dbt relationships) rather than letting the tool infer them.
...
- **Starflake, not pure snowflake.** Flatten by default. Snowflake a dimension only when it meets at least two of: >10M rows, multi-team ownership, high attribute churn, or shared across ≥3 facts. Pure textbook snowflake (every dimension normalized) is almost never the right physical model on a columnar engine.
...
- **Resolve SCD versions at load, snapshot keys into the fact.** Push temporal complexity to ETL so the query layer never needs range joins across snowflaked levels. (See [SCD Types](../scd-types/README.md).)
...
| Situation | Use flat (star) | Snowflake the dimension | Consider [Data Vault](../data-vault/README.md) |
|---|---|---|---|
...
- "For the raw integration layer with heavy source churn I'd reach for [Data Vault](../data-vault/README.md), not a snowflake. Snowflake is a dimensional-serving decision."

## Further reading

- Sibling chapters: [Star Schema](../star-schema/README.md) · [SCD Types](../scd-types/README.md) · [Data Vault](../data-vault/README.md) · [Customer 360](../customer-360/README.md)
