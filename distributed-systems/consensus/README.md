...
- Consensus gives you a **single, totally-ordered, durable log of decisions** even though nodes crash and the network drops/reorders/delays messages. Everything else (leader election, locks, config, KV stores, table commits) is built on that log.
- The real workhorses are **Raft** and **Multi-Paxos** (leader-based, log-replication consensus). The classic single-decree Paxos is a teaching tool; nobody runs it raw in prod.
...
- Consensus is **CP under partition** — it sacrifices availability to never split-brain. A minority partition correctly *refuses to make progress*. See [cap-theorem](../cap-theorem/README.md).
- Consensus is **slow and expensive per decision** (a round trip to a majority on the write path). The principal-level skill is keeping it off the hot path: batch, pipeline, and only run consensus on metadata, never on every row.
- The failures that page you are rarely "the algorithm is wrong." They're **clock skew, fsync lies, undersized disks on the leader, lease/timeout misconfig, and even-node clusters** — operational, not theoretical.
...
1. **Kafka** elects a controller and per-partition leaders via KRaft (Raft-based) or the older ZooKeeper (ZAB, a Paxos cousin). When a broker dies, a new leader for its partitions must be agreed before producers/consumers can make progress.
2. **The Iceberg catalog** serializes table commits. Two concurrent writers both want to add a snapshot; consensus (or an atomic compare-and-swap backed by it) decides who wins and who retries.
...
Now the failure: a broker hosting partition leaders has a flapping NIC. It's not dead — it's *slow*. ZooKeeper/KRaft sessions time out, leadership re-elects, the broker recovers, re-joins, and you get repeated leader churn. Your consumer lag climbs, Spark micro-batches stall waiting on metadata fetches, and your Iceberg commits start failing with `CommitFailedException` because the metadata location moved underneath you. None of these components is "down." Consensus is doing exactly what it's designed to do — refusing to let a flaky node corrupt the agreed state — and the symptom surfaces three layers up as pipeline lag.
...
The core abstraction is **replicated state machine (RSM)**: every node runs the same deterministic state machine, and if you feed every node the *same ordered sequence of commands*, they all end up in the same state. Consensus is the protocol that builds that single ordered sequence — the **replicated log** — across unreliable nodes.
...
2. **Log replication** — the leader appends entries, ships them to followers, and waits for a majority to acknowledge before *committing*.
...
Leader election in Raft uses **randomized timeouts**: each follower waits a random interval (e.g. 150–300ms) for a heartbeat; whoever times out first becomes a candidate, bumps the **term** (a monotonic logical clock), and requests votes. Randomization breaks symmetry so you don't get endless split votes. A candidate wins with majority votes *and* an up-to-date log.
...
"A majority has it durably on disk" means the log entry survived an `fsync`/`fdatasync`. If your consensus node acks before the data hits stable storage — because it's batching, because the OS page cache hasn't flushed, or because a virtualized/cloud disk acks the write before the underlying device does — then a correlated power loss across the quorum can lose a *committed* entry. That's not a consensus bug; it's a broken durability assumption underneath consensus. This is why etcd ships `wal_fsync_duration_seconds` and why a disk that suddenly does 200ms fsyncs (noisy neighbor, EBS throttling) manifests as **leader election storms**: the leader can't fsync its log fast enough to send heartbeats, followers time out, and you re-elect on a loop.

> **Operational tell:** rising `p99 fsync latency` on the leader followed by `leader changed` events. The fix is faster/dedicated disk (provisioned IOPS, local NVMe for the WAL), not a higher election timeout.

### 2. Leases, not just leadership, prevent stale reads

A subtle one: a node can *think* it's the leader after it has actually been deposed (it was partitioned, a new term elected a new leader, the old leader hasn't noticed). If that stale leader serves a read from its local state, it can return data older than a committed write — violating linearizability. Two fixes:
...
- **Leader leases**: the leader holds a time-bounded lease; within it, it serves reads locally. This is *only* safe if you bound clock drift between nodes (which is why leases are tied to clock-skew assumptions — get this wrong and you serve stale reads). etcd and CockroachDB both lean on clock bounds here.

### 3. Membership changes are where clusters split-brain

Adding/removing a node naively can create a window where two disjoint majorities exist (old config majority and new config majority don't overlap). Raft solves this with **single-server changes** (add/remove one node at a time, so consecutive configs always overlap) or **joint consensus** (a transitional config requiring both old and new majorities). The classic outage: someone "replaces" 3 nodes by spinning up 3 new ones and pointing them at the cluster, briefly creating a 6-node membership with two possible 3-node majorities → split brain → divergent logs. **Never bulk-replace consensus members.** Drain one, add one, repeat.
...
Every committed write is at least one round trip from leader to the nearest majority. Spread a 3-node cluster across three regions for "durability" and you've made every write pay cross-region RTT (50–100ms+). This is why control planes are kept AZ-local-with-fast-links and why you **never put consensus on the data hot path**. Kafka doesn't run consensus per message — it runs ISR replication for data and reserves Raft/ZAB for *metadata* (leadership, configs, topic state). Iceberg doesn't run consensus per row — it runs one CAS per *commit* (a whole snapshot of many files).

### 6. Byzantine vs crash-fault

Raft/Paxos assume **crash-stop / fail-recover**: nodes can die or pause but don't lie. They are *not* Byzantine fault tolerant — a malicious or arbitrarily-corrupted node can break them. BFT consensus (PBFT, Tendermint) needs `3f+1` nodes to tolerate `f` Byzantine faults and is the domain of blockchains, not internal data platforms. For a data engineer inside a trusted VPC, crash-fault consensus is correct and BFT is overkill.
...
Iceberg doesn't run Raft itself — it pushes the "agree on one winner" problem down to the catalog, which must provide an **atomic CAS** on the table's current metadata pointer. That atomicity is what's backed by a consensus system (the metastore DB's transaction, a DynamoDB conditional write, a Nessie/Raft commit). The writer wraps it in an optimistic retry loop:
...
- **Keep consensus on metadata, ISR/quorum-replication on data.** Mirror what Kafka and Iceberg do: agree on *who's in charge* and *what the current pointer is* via consensus; replicate the bulk bytes via cheaper mechanisms. Never run a consensus round per record.
...
- **Tune election timeout to your network, then leave it.** Rule of thumb: `election-timeout ≥ 10 × heartbeat` and `heartbeat ≥ observed p99 one-way delay`. Cross-AZ jitter of a few ms with a 1000ms election timeout is comfortable; the same with a 200ms timeout flaps.
...
- **Decide your read consistency explicitly.** Linearizable reads (ReadIndex) for correctness-critical paths; lease/local reads for throughput where bounded staleness is acceptable. Document which you chose — see [consistency-models](../consistency-models/README.md).
...
| Election timeout too aggressive for the topology | Constant re-elections under normal network jitter; no node "dead" | Raise election-timeout to ≥10× heartbeat and above p99 one-way delay |
...
| Mutual exclusion / distributed lock | Consensus-backed lease **with a fencing token** | Best-effort lock with no fencing (lock expires, two holders act) |
...
The meta-decision: **do you actually need linearizable agreement, or do you need durability + ordering you can get more cheaply?** Most data-plane problems are the latter. Consensus is the right tool when *more than one writer could disagree about a single piece of authoritative state* — leadership, a table pointer, a lock, a config epoch. If you find yourself reaching for consensus on the per-row write path, step back; you almost certainly want quorum replication or idempotent writes instead. See [event-driven-systems](../event-driven-systems/README.md).
...
- Diego Ongaro & John Ousterhout, *In Search of an Understandable Consensus Algorithm (Raft)*, USENIX ATC 2014 — the paper to read; readable in an afternoon.
- Leslie Lamport, *Paxos Made Simple* (2001) — the foundation; pair it with *Paxos Made Live* (Chandra et al., Google, 2007) for the operational reality.
- The etcd documentation on tuning, disaster recovery, and the `--force-new-cluster` recovery path — the closest thing to a production consensus operations manual.
