...
- **Structured JSON logs with a stable schema** beat free-text. You query logs more than you read them; `WHERE job_run_id = ... AND level = 'ERROR'` only works if `job_run_id` is a field, not a substring.
...
- **Executor logs are the hard part.** Driver logs are easy and small; the truth about data lives in executor stdout/stderr, which is ephemeral, sharded across nodes, and gone when the cluster scales in. Ship them before the node dies.
- **Logging is a cost center at scale.** Per-record `logger.info` inside a `mapPartitions` is how you turn a 2 TB job into a 40 GB log bill and a serialized-logger `NotSerializableException`.
- **Logs, metrics, and traces are different tools.** Don't compute aggregates by grep-ing logs; that belongs in [metrics](../metrics/README.md). Logs answer "what happened to *this* thing," not "how often does X happen."
...
The DAG says success. The Spark History Server says the job ran in 22 minutes. CloudWatch says the EMR cluster terminated cleanly. Everything is green and the data is wrong.

What actually happened: a malformed upstream Avro file caused a `from_json` to return `null` for 8% of rows, a `WHERE col IS NOT NULL` filter silently dropped them, and the job "succeeded" with 8% fewer rows. No exception was thrown. The only evidence that ever existed was an executor `WARN` line — `Corrupt record in partition s3://.../dt=2026-06-17/part-00131.avro` — emitted once per bad partition, on an executor that was reclaimed by autoscaling 18 minutes ago. The log is gone.
...
1. **Emission.** A logging facade (SLF4J in JVM, Python `logging`) hands a record to a backend (Log4j2, `structlog`). The backend applies a *layout* that decides text vs. JSON, and which fields (timestamp, level, logger name, thread, MDC context) get attached.

2. **Collection.** On Spark, driver logs go to the launching process; executor logs go to per-container files (YARN `container_*/stdout`, or local dirs on K8s/standalone). A shipper tails these and forwards them. The critical, often-missed fact: **on autoscaled clusters, executor log files are deleted when the node terminates** unless you've configured log aggregation (`yarn.log-aggregation-enable=true`, or `spark.kubernetes.executor.deleteOnTermination=false` plus a sidecar shipper, or EMR's S3 log push).

3. **Storage & query.** Hot, searchable storage (OpenSearch/Loki/Datadog) is expensive; cold object storage (S3 + Athena/Glue) is cheap but slow. The standard split is hot for the last 7–14 days, cold (S3, partitioned by `dt`) for retention beyond that.

### The MDC / context-propagation mechanism

Structured logging hinges on a **Mapped Diagnostic Context** (MDC): a thread-local key-value map that the layout automatically merges into every log line. You set `MDC.put("run_id", runId)` once and every subsequent `logger.info` on that thread carries it. The trap in Spark: MDC is *thread-local*, and your code runs on executor threads that never saw the driver's MDC. You must propagate context into the closure explicitly (capture it in a variable, log it as a field) — the driver's MDC does not magically follow a task to an executor.
...
The right executor-side pattern is to **aggregate within the partition and emit once**:
...
A second executor trap: **logger serialization.** In Scala/Java, a `private val logger = LoggerFactory.getLogger(...)` referenced inside a closure tries to serialize the enclosing object → `org.apache.spark.SparkException: Task not serializable` or a `NotSerializableException`. Fix: make the logger `@transient lazy val`, or fetch it inside the closure. In PySpark, never capture a `logging.Logger` object in a closure; call `logging.getLogger(name)` *inside* the executor function.
...
Stamp `run_id` into the Iceberg commit as well (`df.writeTo(t).option("snapshot-property.run-id", run_id)`), so [lineage](../lineage/README.md) can walk from a wrong row → snapshot → run → logs without guessing.

### Log4j2 JSON, the EMR/Spark-specific way

Text logs force you to regex `2026-06-18 03:14:02 WARN dq corrupt_records...`, which breaks the moment someone changes the layout. JSON makes every field queryable. The Spark-correct way is a `log4j2.properties` (Spark 3.3+ defaults to Log4j2) with a JSON layout:
...
Cutting `org.apache.spark` and `org.apache.hadoop` from `INFO` to `WARN` alone routinely removes 80–95% of executor log volume — the `BlockManager`, `MemoryStore`, and `MapOutputTracker` chatter that nobody queries.

## Worked example

End-to-end: structured logging in a PySpark job with full correlation context, partition-level DQ aggregation, and redaction. Runnable shape on EMR/Glue/Databricks.
...
- **Ship executor logs off-node before scale-in.** On EMR set `s3://.../logs` log push; on Spark-on-K8s run Fluent Bit/Vector as a sidecar or DaemonSet; on YARN enable log aggregation. Verify by intentionally killing an executor and confirming its logs survived.
- **Two-tier retention.** Hot store (OpenSearch/Datadog/Loki) for 7–14 days for active debugging; cold S3 partitioned by `dt`, queryable via Athena, for the 90-day–7-year compliance tail. Set OpenSearch ILM / Datadog retention explicitly — defaults are expensive.
- **Sample `INFO`, never sample `WARN`/`ERROR`.** High-volume lifecycle logs can be sampled (e.g., 1-in-N micro-batches in Structured Streaming). Anything signaling degradation or failure is always kept.
...
- **Emit a machine-readable run summary as the last log line.** A single `run_summary` JSON event (rows in/out, duration, dropped count, output snapshot id) gives [monitoring](../monitoring/README.md) and [metrics](../metrics/README.md) a clean hook without parsing the whole stream.
...
| Computing metrics by grep-ing logs | Slow, expensive, drifts from reality; alerts lag by minutes | Emit real [metrics](../metrics/README.md); logs are for the single-entity story |
...
A subtle streaming-specific failure: in Spark Structured Streaming, a `logger.info` in `foreachBatch` fires once per micro-batch — fine at a 1-minute trigger, catastrophic at a 100 ms trigger. Tie streaming log verbosity to the trigger interval, or sample by `batchId % N == 0`.
...
| "How often / how much / how fast, over time?" | **Metrics** | Cheap aggregates, alertable, low cardinality — see [metrics](../metrics/README.md) |
| "Where did the latency / data go across services?" | **Traces / lineage** | Causal path across hops — see [lineage](../lineage/README.md) |
...
| OpenSearch / Elastic | Fast interactive search, dashboards | Cost & cluster ops scale with hot data volume |
| Grafana Loki | Label-indexed logs alongside Prometheus metrics | Full-text search weaker than OpenSearch |
| Datadog / Splunk | Turnkey, correlated with APM | Ingest-based pricing punishes volume mistakes hard |
...
- [Metrics](../metrics/README.md) — when the question is "how often / how much," not "what happened to this one thing."
- [Monitoring & alerting](../monitoring/README.md) — turning `ERROR` rates and `run_summary` events into pages that matter.
- [Lineage](../lineage/README.md) — walking from a wrong row → Iceberg snapshot → run → the logs that explain it.
- [Data quality: completeness](../../data-quality/completeness/README.md) — the silent-row-drop failure class that `WARN`-with-counts surfaces.
- [Iceberg](../../lakehouse/iceberg/README.md) — snapshot properties as the join key between commits and logs.
- Google SRE Book, *Monitoring Distributed Systems* — the logs/metrics/traces distinction and the four golden signals.
- Elastic Common Schema (ECS) — a stable, queryable JSON field convention worth adopting wholesale rather than inventing your own.
