...
- Strategy is a **constraint system**, not a vision deck. Its output is a small set of invariants ("all batch state lives in Iceberg tables in `s3://lake/`, never in HDFS or a sidecar RDBMS") that make 80% of downstream decisions automatic.
...
1. *Scaffolding* — the project template / cookiecutter emits Iceberg DDL by default, so the path of least resistance is compliant.
2. *CI guardrail* — a lint check fails any PR that registers a non-Iceberg external table in the production catalog.
3. *Telemetry* — a dashboard counts `non_iceberg_table_count` in prod and trends it to zero.

**The reversibility frame.** Borrowing Bezos's one-way/two-way door language, but quantified for data:
...
**5. Migrations are the real strategy, and they must be designed to be abandonable mid-flight.** The strategy is rarely "be on Iceberg" — it's "*get* from Hive-Parquet to Iceberg without a flag day." A migration that requires a big-bang cutover is a strategy that can't survive a single bad quarter. Design every migration with a dual-write or shadow-read phase and a documented rollback, so that if priorities shift you stop in a *consistent* state rather than stranded half-migrated across two formats forever — which is exactly how orgs end up with the two-table-format problem from the opening scenario.

**6. The build-vs-buy decision is about who carries the pager.** Feature parity is the least interesting axis. The real axes:
...
- **Strangler-fig migrations with a kill switch.** Route a slice of traffic/tables to the new system behind a flag, shadow-read to compare, ramp gradually, and keep the old path warm until the new one holds for N weeks. Never a flag day.
...
- **Tracer-bullet the riskiest assumption first.** If the strategy depends on "Iceberg snapshot expiry will keep metadata manageable at 50k commits/day," prove *that* in week one with a load test, not in month six in production.
...
- **One-way doors made implicitly.** *Symptom:* an event schema shipped without review; six months later three external consumers depend on a misnamed field you can't change. *Fix:* gate all wire-format/schema changes behind compatibility checks and a design review (see [kafka/event-design](../../kafka/event-design/README.md)).
...
- Werner Vogels / Jeff Bezos, "one-way vs. two-way doors" — the reversibility frame applied to architecture decisions.
- Melvin Conway (1968), "How Do Committees Invent?" — the original statement of Conway's Law, the most under-used strategic input in data engineering.
- Michael Nygard, "Documenting Architecture Decisions" (2011) — the canonical ADR pattern.
